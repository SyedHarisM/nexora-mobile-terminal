...
const handleLogin = async () => {
    ...
    try {
      ...
      if (response.data?.accessToken) {
        ...
        // 🟩 CRITICAL FIX: Save proper userData object for Profile Screen to read instantly
        const userObj = {
          id: response.data.id || "???",
          email: identity.trim().toLowerCase(),
          name: response.data.name || "Nexora User",
          username: response.data.username || "cyber_node",
          accountNumber: response.data.accountNumber || "Not Linked"
        };

        await AsyncStorage.setItem('user_name', response.data.name || "NEXORA OPERATOR");
        await SecureStore.setItemAsync('userToken', token);
        
        // 🟩 PROFILE SYNC FIX: Is line se profile screen ko data milega
        await SecureStore.setItemAsync('userData', JSON.stringify(userObj));
        
        // Always save these for Biometric trigger
